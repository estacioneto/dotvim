...
local function get_remote()
  ...
  handle:close()
  local _, _, domain, _, group, repo =
    result:match '(ssh://)([^@]+)@([^:]+):(%d+)/([^/]+)/([^/]+).git'

  return ('https://%s/projects/%s/repos/%s/browse/'):format(
    domain,
    group:upper(),
    repo
  )
...
local function git_update_branch(opts)
  ...
  utils.run_async_cmd(
    'git',
    { title = 'Git', args = { 'checkout', branch } },
    function(checkout_code)
      ...
      utils.run_async_cmd(
        'git',
        { title = 'Git', args = { 'pull', 'origin', branch, '-pr' } },
        function(pull_code)
          if pull_code ~= 0 then
            return
          end
          ...
          utils.run_async_cmd(
            'git',
            { title = 'Git', args = { 'checkout', current_branch } }
          )
        end
      )
    end
...
local function git_rebase(opts)
  ...
  utils.run_async_cmd(
    'git',
    { title = 'Git', args = { 'rebase', branch, '--no-verify' } }
  )
end
